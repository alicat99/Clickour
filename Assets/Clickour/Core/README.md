...
Game Scene의 `Cursor`에는 `Rigidbody2D`, `Collider2D`, `CollisionExitGate`, `CursorController`, `CursorInteractor`가 있다. Play Mode에서 좌우 방향키 또는 A/D를 누르면 커서가 수평 가속으로 이동하며 중력과 파란 발판 충돌은 유지된다. 클릭 가능한 두 Collider가 동시에 후보일 때, 접촉 중인 후보가 겹친 후보보다 먼저 선택된다. Edit Mode Test Runner의 `ClickSelectionPolicyTests`와 `DirectionalInputTests`가 선택 우선순위와 방향 입력 조합을 검증한다.
...
| `DirectionalInput.cs` | 방향키와 WASD를 동일한 2축 입력으로 통합 |
