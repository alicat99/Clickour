...
외형은 Material 3의 52×32dp 트랙과 원형 handle anatomy를 따른다. Game Scene의 `Clickour/Capture Game View` 캡처에서 파란 비활성 트랙의 얇은 outline, 활성/회색 트랙의 외곽선 없는 solid fill, 외곽선 없는 handle과 회색/파란 상태색을 확인한다. 활성 handle은 트랙 높이의 약 75%, 비활성 handle은 약 50%이며 토글할 때 위치와 크기가 함께 전환된다. 검증 Scene의 가로 파란 스위치는 활성, 세로 파란 스위치는 비활성으로 시작해 두 상태를 한 캡처에서 비교한다. 색은 게임 규칙 때문에 M3 토큰의 역할을 짙은 회색 clickable과 Google blue 계열 collidable에 매핑한다.
...
```csharp
mechanic.Configure(SwitchKind.Blue, handle, track, checked_track, solid, exit_zone);
```
...
| `Art/switch_track.svg` | 비활성 outline 트랙 시각 원본 |
| `Art/switch_track_checked.svg` | 활성 solid 트랙 시각 원본 |
